...
describe('Track Keyword Positions Job Integration', function (): void {
    ...
    describe('Job Dispatch and Execution', function (): void {
        it('dispatches job for project successfully', function (): void {
            $keywords = Keyword::factory()->count(5)->for($this->project)->for($this->tenant)->create();

            TrackKeywordPositionsJob::dispatch($this->project);

            Queue::assertPushed(TrackKeywordPositionsJob::class, function ($job): bool {
                return $job->project->id === $this->project->id;
            });
        });

        it('processes keywords in batches', function (): void {
            ...
            expect(KeywordPosition::query()->where('tenant_id', $this->tenant->id)->count())->toBe(25);
        });

        it('updates project last_positions_updated_at timestamp', function (): void {
            ...
            expect($this->project->last_positions_updated_at)->toBeInstanceOf(Carbon::class);
        });

        it('handles only active keywords', function (): void {
            ...
            expect(KeywordPosition::query()->count())->toBe(3);
        });
    });

    describe('Error Handling and Retry Logic', function (): void {
        it('handles API rate limiting with retry', function (): void {
            ...
            expect(KeywordPosition::query()->count())->toBeGreaterThan(0);
        });

        it('handles partial API failures gracefully', function (): void {
            $keywords = Keyword::factory()->count(5)->for($this->project)->for($this->tenant)->create();
            ...
            expect(KeywordPosition::query()->count())->toBe(4);
        });

        it('fails job after maximum retries', function (): void {
        ...
        it('handles network timeouts with exponential backoff', function (): void {
            $keywords = Keyword::factory()->count(2)->for($this->project)->for($this->tenant)->create();
    ...
    describe('Performance and Resource Management', function (): void {
        it('processes large keyword sets efficiently', function (): void {
            ...
            expect(KeywordPosition::query()->count())->toBe(100);
            expect($duration)->toBeLessThan(30); // Should complete within 30 seconds
        });

        it('manages memory usage for large datasets', function (): void {
        ...
        it('respects API rate limits', function (): void {
    ...
    describe('Data Integrity and Consistency', function (): void {
        it('ensures transactional integrity', function (): void {
            $keywords = Keyword::factory()->count(5)->for($this->project)->for($this->tenant)->create();
            ...
            try {
                ...
                // Job should fail
                expect($exception->getMessage())->toContain('Critical error');
            }

            // No positions should be saved due to transaction rollback
            expect(KeywordPosition::query()->count())->toBe(0);
        });

        it('prevents duplicate position entries', function (): void {
            ...
            expect(KeywordPosition::query()->where('keyword_id', $keyword->id)
                ->where('date', now()->format('Y-m-d'))
                ->count())->toBe(1);
        });

        it('maintains data consistency across tenants', function (): void {
            ...
            $tenant1Positions = KeywordPosition::query()->where('tenant_id', $this->tenant->id)->count();
            $tenant2Positions = KeywordPosition::query()->where('tenant_id', $tenant2->id)->count();
    ...
    describe('Monitoring and Notifications', function (): void {
        it('logs job progress and metrics', function (): void {
            $keywords = Keyword::factory()->count(10)->for($this->project)->for($this->tenant)->create();
        ...
        it('creates notifications for significant changes', function (): void {
            $keyword = Keyword::factory()->for($this->project)->for($this->tenant)->create([
                'current_position' => 15,
            ]);
        ...
        it('sends alerts for ranking losses', function (): void {
            $keyword = Keyword::factory()->for($this->project)->for($this->tenant)->create([
    ...
    describe('Job Queuing and Scheduling', function (): void {
        it('can be scheduled for automatic execution', function (): void {
            $keywords = Keyword::factory()->count(5)->for($this->project)->for($this->tenant)->create();
        ...
        it('can be queued with priority', function (): void {
        ...
        it('handles job uniqueness to prevent duplicates', function (): void {
            $keywords = Keyword::factory()->count(3)->for($this->project)->for($this->tenant)->create();
    ...
    describe('Cleanup and Maintenance', function (): void {
        it('cleans up old position data', function (): void {
            $keyword = Keyword::factory()->for($this->project)->for($this->tenant)->create();
            ...
            expect(KeywordPosition::query()->where('date', '<', now()->subDays(60))->count())->toBe(0);
            expect(KeywordPosition::query()->where('date', '>=', now()->subDays(60))->count())->toBeGreaterThan(0);
        });
    });
});
