...
final class TenantPermissionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $superTenant = Tenant::query()->firstOrCreate(['slug' => 'super-admin'], [
            'uuid' => Str::uuid(),
            'name' => 'Super Admin',
            'plan' => 'enterprise',
            'max_projects' => 999999,
        ...
        $superAdmin = User::query()->firstOrCreate(['email' => 'admin@seodashboard.local'], [
        ...
        $demoTenant = Tenant::query()->firstOrCreate(['slug' => 'demo-company'], [
            'uuid' => Str::uuid(),
            'name' => 'Demo Company',
            'plan' => 'professional',
            'max_projects' => 10,
        ...
        // Create demo users
        $demoOwner = User::query()->firstOrCreate(['email' => 'owner@demo.local'], [
        ...
        $demoManager = User::query()->firstOrCreate(['email' => 'manager@demo.local'], [
            'tenant_id' => $demoTenant->id,
            'name' => 'Demo Manager',
        ...
        $demoViewer = User::query()->firstOrCreate(['email' => 'viewer@demo.local'], [
            'tenant_id' => $demoTenant->id,
            'name' => 'Demo Viewer',
    ...
    protected function setupTenantPermissions(Tenant $tenant, User $owner): void
    {
        ...
        $roles = [
            ...
            'admin' => [
                ...
                'permissions' => array_filter(array_keys($permissions), function (string $perm): bool {
        ...
        foreach ($roles as $roleName => $roleData) {
            ...
            $rolePermissions = Permission::query()->where('tenant_id', $tenant->id)
        ...
        $ownerRole = Role::query()->where('name', 'owner')
     ...
     */
    protected function assignUserRoles(Tenant $tenant, array $assignments): void
    {
        foreach ($assignments as $assignment) {
            $role = Role::query()->where('name', $assignment['role'])
                ->where('tenant_id', $tenant->id)
                ->first();
