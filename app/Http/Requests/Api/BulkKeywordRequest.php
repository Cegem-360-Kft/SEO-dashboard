...
class BulkKeywordRequest extends FormRequest
{
        ...
        if ($this->keywords && is_array($this->keywords)) {
            ...
            foreach ($this->keywords as $index => $keyword) {
                ...
                if (isset($keyword['term'])) {
                    $cleanTerm = trim(strtolower($keyword['term']));
        ...
        $validator->after(function ($validator) {
            if ($this->keywords && is_array($this->keywords)) {
                    ...
                    if ($term) {
                        ...
                        if (preg_match('/[<>{}"\']/', $term)) {
                            $validator->errors()->add(
                                "keywords.{$index}.term",
                                "Keyword '{$term}' contains invalid characters."
                            );
                        }
                        ...
                        if (str_word_count($term) > 10) {
                            $validator->errors()->add(
                                "keywords.{$index}.term",
                                "Keyword '{$term}' cannot exceed 10 words."
                            );
                        }
                        ...
                        if ($searchVolume > 0) {
                            ...
                            if ($termLength > 5 && $searchVolume > 100000) {
                                $validator->errors()->add(
                                    "keywords.{$index}.search_volume",
                                    "Search volume for '{$term}' seems unusually high for a long-tail keyword."
                                );
                            }
                        }
                    }
                ...
                if ($keywordCount > 500) {
                    $validator->errors()->add(
                        'keywords',
                        "Processing {$keywordCount} keywords may take a while. Consider splitting into smaller batches for better performance."
                    );
                }
            }
        });
    ...
    public function validated($key = null, $default = null): array
    {
        ...
        if (isset($validated['keywords'])) {
            $validated['keywords'] = array_filter($validated['keywords'], function ($keyword) {
