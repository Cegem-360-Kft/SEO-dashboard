...
class PositionOverviewWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        for ($i = 29; $i >= 0; $i--) {
            ...
            $dayTop10 = Keyword::where('tenant_id', $tenantId)
                ->whereBetween('current_position', [1, 10])
                ->whereDate('position_last_updated', $date)
                ->count();
            
            $dayTop20 = Keyword::where('tenant_id', $tenantId)
                ->whereBetween('current_position', [1, 20])
                ->whereDate('position_last_updated', $date)
                ->count();
                
            $dayTotal = Keyword::where('tenant_id', $tenantId)
        ...
        return [
            'datasets' => [
                ...
                [
                    ...
                    'data' => $data->map(fn (TrendValue $value) => $value->aggregate ? round($value->aggregate, 1) : 0)->toArray(),
                    'borderColor' => '#3b82f6',
            ...
            'labels' => $data->map(fn (TrendValue $value) => Carbon::parse($value->date)->format('M j'))->toArray(),
        ];
    }
