...
final class QuickStatsWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        // Total projects
        $totalProjects = Project::query()->where('tenant_id', $tenantId)->count();
        $activeProjects = Project::query()->where('tenant_id', $tenantId)->where('is_active', true)->count();

        // Total keywords
        $totalKeywords = Keyword::query()->where('tenant_id', $tenantId)->count();
        $activeKeywords = Keyword::query()->where('tenant_id', $tenantId)
            ->where('is_tracking_active', true)
            ->count();

        // Position stats
        $top10Keywords = Keyword::query()->where('tenant_id', $tenantId)
            ->whereBetween('current_position', [1, 10])
            ->count();

        $top3Keywords = Keyword::query()->where('tenant_id', $tenantId)
            ->whereBetween('current_position', [1, 3])
            ->count();

        // Average position
        $averagePosition = Keyword::query()->where('tenant_id', $tenantId)
            ->whereNotNull('current_position')
            ->avg('current_position');

        // Trending keywords (improving vs declining)
        $improvingKeywords = Keyword::query()->where('tenant_id', $tenantId)
            ->whereRaw('previous_position > current_position')
            ->whereNotNull('previous_position')
            ->whereNotNull('current_position')
            ->count();

        $decliningKeywords = Keyword::query()->where('tenant_id', $tenantId)
            ->whereRaw('previous_position < current_position')
            ->whereNotNull('previous_position')
            ->whereNotNull('current_position')
            ->count();
