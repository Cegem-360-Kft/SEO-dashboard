...
class ReportingService
{
    ...
    public function generateReport(Project $project, array $options = []): Report
    {
        try {
            ...
            $reportData = $this->collectReportData($project, $startDate, $endDate, $options);
            
            // Generate report content
            $content = $this->generateReportContent($reportData, $options);
     ...
     */
    private function collectReportData(Project $project, $startDate, $endDate, array $options): array
    {
        $data = [
            'project_info' => [
                'id' => $project->id,
        ...
        $keywords = $project->keywords()->with(['positions' => function($query) use ($startDate, $endDate) {
            $query->whereBetween('tracked_at', [$startDate, $endDate])
                  ->orderBy('tracked_at', 'desc');
        }])->get();
        
        $data['keywords'] = $keywords->map(function($keyword) {
            ...
            return [
                ...
                'trends' => $trends,
                'position_history' => $keyword->positions->map(function($position) {
                    return [
     ...
     */
    private function generateReportContent(array $data, array $options): array
    {
        $content = [];
        
        // Generate HTML content
        $content['html'] = view('reports.templates.seo-report', $data)->render();
        
        // Generate executive summary
        $content['executive_summary'] = $this->generateExecutiveSummary($data);
        
        // Generate key insights
        $content['insights'] = $this->generateKeyInsights($data);
    ...
    private function generateExecutiveSummary(array $data): string
    {
        ...
        $text .= "• Average Position: {$summary['average_position']}\n";
        $text .= "• Keywords in Top 10: {$summary['top_10_keywords']}\n";
    ...
    private function generateKeyInsights(array $data): array
    {
        ...
        if ($summary['average_position'] <= 10) {
            $insights[] = [
                ...
                'description' => "Average position of {$summary['average_position']} indicates strong ranking performance.",
            ];
            ...
            $insights[] = [
                ...
                'description' => "Average position of {$summary['average_position']} requires immediate optimization efforts.",
            ];
        }
        ...
        if ($summary['featured_snippets'] > 0) {
            $insights[] = [
                ...
                'description' => "Secured {$summary['featured_snippets']} featured snippets, providing enhanced visibility.",
            ];
        }
    ...
    {
        $reportPath = "reports/{$report->tenant_id}/{$report->id}";
    ...
    private function getCompetitorAnalysis(Project $project): array
    {
        ...
        usort($analysis, fn($a, $b) => $a['average_position'] <=> $b['average_position']);
    ...
    private function countBetterPositions(Project $project, $competitor): int
    {
        ...
        foreach ($project->keywords as $keyword) {
            ...
            $competitorPosition = $competitor->keywordPositions()
                ->whereHas('keyword', function($query) use ($keyword) {
    ...
    public function deleteReport(Report $report): bool
    {
        try {
            ...
            if ($report->file_paths) {
                foreach ($report->file_paths as $format => $path) {
    ...
    public function generateCustomReport(Project $project, array $template, array $options = []): Report
    {
        try {
            ...
            $reportData = $this->processCustomTemplate($project, $template, $options);
     ...
     */
    private function processCustomTemplate(Project $project, array $template, array $options): array
    {
        $data = [];
        
        foreach ($template['sections'] ?? [] as $section) {
            $sectionData = $this->processSectionData($project, $section, $options);
     ...
     */
    private function processSectionData(Project $project, array $section, array $options): array
    {
        return match($section['type']) {
            ...
            'competitor_comparison' => $this->getCompetitorComparisonData($project, $section['config'] ?? []),
            'technical_seo' => $this->getTechnicalSeoData($project, $section['config'] ?? []),
            default => []
        };
    }
    ...
    private function getKeywordPerformanceData(Project $project, array $config): array
    {
        $keywords = $project->keywords()
            ->with('positions')
            ->when($config['limit'] ?? null, function($query, $limit) {
                $query->limit($limit);
            })
            ->get();
     ...
     */
    private function getCompetitorComparisonData(Project $project, array $config): array
    {
     ...
     */
    private function getTechnicalSeoData(Project $project, array $config): array
    {
