...
class SEOCalculationService
{
    ...
    public function calculateTrafficPotential(Project $project): array
    {
        ...
        usort($potential['improvement_opportunities'], 
            fn($a, $b) => $b['improvement_potential'] <=> $a['improvement_potential']
        );
     ...
     */
    public function calculateKeywordDifficulty(string $keyword, string $domain): float
    {
        ...
        $factors = [
            ...
            'competition_level' => $this->analyzeCompetitionLevel($keyword),
        ];
     ...
     */
    private function analyzeCompetitionLevel(string $keyword): float
    {
    ...
    public function generateRecommendations(Project $project, array $reportData = []): array
    {
        ...
        $recommendations[] = $this->generateTechnicalSeoRecommendations($project);
     ...
     */
    private function generateTechnicalSeoRecommendations(Project $project): array
    {
        return [
            'priority' => 'medium',
    ...
    private function generateCompetitorRecommendations(array $competitors): array
    {
        ...
        if ($strongestCompetitor && $strongestCompetitor['better_positions'] > 5) {
            $recommendations[] = [
                ...
                'description' => "{$strongestCompetitor['domain']} outranks you on {$strongestCompetitor['better_positions']} keywords.",
                'action_items' => [
    ...
    public function calculateContentOpportunities(Project $project): array
    {
        ...
        usort($opportunities, fn($a, $b) => $b['traffic_potential'] <=> $a['traffic_potential']);
    ...
    public function calculateSerpFeatureOpportunities(Project $project): array
    {
        ...
        foreach ($topKeywords as $keyword) {
            $hasFeaturedSnippet = $keyword->positions()
                ->whereHas('serpFeatures', function($query) {
            ...
            if (!$hasFeaturedSnippet && $keyword->latest_position <= 5) {
                $opportunities[] = [
                    ...
                    'current_position' => $keyword->latest_position,
                    'opportunity_score' => $this->calculateFeatureOpportunityScore($keyword, 'featured_snippet'),
                ];
            }
        }
     ...
     */
    private function calculateFeatureOpportunityScore(Keyword $keyword, string $featureType): int
    {
        $score = 0;
