...
class AnalyticsService
{
    ...
    private function initializeGoogleClient(): void
    {
        try {
            ...
            Log::error('Failed to initialize Google API client', ['error' => $e->getMessage()]);
        }
    }
    ...
    public function getSearchConsoleData(Project $project, array $options = []): array
    {
        ...
        $cacheKey = "gsc_data:{$project->id}:{$startDate}:{$endDate}";
        ...
        try {
            ...
            if ($response->getRows()) {
                ...
                // Sort by performance
                usort($data['queries'], fn($a, $b) => $b['clicks'] <=> $a['clicks']);
                usort($data['pages'], fn($a, $b) => $b['clicks'] <=> $a['clicks']);
    ...
    public function getAnalyticsData(Project $project, array $options = []): array
    {
        ...
        $cacheKey = "ga4_data:{$project->id}:{$startDate}:{$endDate}";
        ...
        try {
            ...
            $metrics = [
                ...
                new \Google\Service\AnalyticsData\Metric(['name' => 'users']),
                new \Google\Service\AnalyticsData\Metric(['name' => 'pageviews']),
                ...
                new \Google\Service\AnalyticsData\Metric(['name' => 'conversions']),
            ];
            ...
            $dimensions = [
                ...
                new \Google\Service\AnalyticsData\Dimension(['name' => 'sessionDefaultChannelGroup']),
                new \Google\Service\AnalyticsData\Dimension(['name' => 'landingPage']),
            ];
            ...
            if ($response->getRows()) {
                ...
                if ($totalRows > 0) {
                    $data['overview']['bounce_rate'] = array_sum(array_column($response->getRows(), function($row) {
                        return (float) $row->getMetricValues()[3]->getValue();
                    })) / $totalRows;
                    
                    $data['overview']['avg_session_duration'] = array_sum(array_column($response->getRows(), function($row) {
                        return (float) $row->getMetricValues()[4]->getValue();
                    })) / $totalRows;
                }
                
                // Sort arrays
                usort($data['traffic_sources'], fn($a, $b) => $b['sessions'] <=> $a['sessions']);
                usort($data['landing_pages'], fn($a, $b) => $b['sessions'] <=> $a['sessions']);
    ...
    private function findMatchingKeyword(string $query, array $keywords): ?string
    {
        ...
        if (in_array($query, array_map('strtolower', $keywords))) {
            return $keywords[array_search($query, array_map('strtolower', $keywords))];
        }
    ...
    {
        $gscData = $this->getSearchConsoleData($project);
        $ga4Data = $this->getAnalyticsData($project);
        ...
        if (!isset($gscData['error'])) {
            ...
            $topKeywords = array_filter($gscData['queries'], fn($q) => $q['position'] <= 10);
        ...
        $featuredSnippets = $project->keywords()
            ->whereHas('positions.serpFeatures', function($query) {
