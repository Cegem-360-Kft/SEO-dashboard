...
class NotificationService
{
    ...
    public function sendReportNotification(Project $project, $report): void
    {
        try {
            $notification = $this->createNotification([
                ...
                'message' => "Your SEO report for {$project->name} has been generated and is ready for download.",
                'data' => [
                    'report_id' => $report->id,
        ...
        foreach ($users as $user) {
            try {
                ...
                if ($notification->priority === 'high' && 
                    ...
                    $user->phone) {
                    $this->sendSmsNotification($user, $notification, $data);
                }
    ...
    {
        try {
            Mail::to($user->email)->send(new \App\Mail\SeoAlert($notification, $data));
     ...
     */
    private function sendSmsNotification(User $user, Notification $notification, array $data): void
    {
        try {
    ...
    private function formatTrafficMessage(array $data): string
    {
        ...
        return "Organic traffic has {$direction} by {$abs_change}% compared to the previous period. " .
    ...
    private function formatCompetitorMessage(array $data): string
    {
        ...
        return "Competitor {$domain} has improved rankings on {$keywordCount} keywords you're tracking. " .
    ...
    private function formatSmsMessage(Notification $notification): string
    {
        ...
        if ($notification->type === 'position_alert') {
            ...
            $message = "SEO Alert: '{$keyword}' now at position {$position}";
        }
        ...
        foreach ($users as $user) {
            try {
                ...
                if (!empty($digestData['projects'])) {
                    Mail::to($user->email)->send(new \App\Mail\DailyDigest($digestData));
        ...
        foreach ($users as $user) {
            try {
                ...
                if (!empty($summaryData['projects'])) {
                    Mail::to($user->email)->send(new \App\Mail\WeeklyPerformanceSummary($summaryData));
    ...
    private function prepareWeeklySummary(Tenant $tenant): array
    {
        $projects = $tenant->projects()
            ->with(['keywords.positions' => function($query) {
    ...
    private function calculateWeeklyProjectPerformance(Project $project): array
    {
        ...
        $data['avg_position_change'] = !empty($positionChanges) 
