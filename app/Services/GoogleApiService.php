...
class GoogleApiService
{
    ...
    public function testAnalyticsConnection(string $propertyId): bool
    {
        ...
        try {
            ...
            $metrics = [new \Google\Service\AnalyticsData\Metric(['name' => 'sessions'])];
    ...
    public function getSearchConsoleData(string $siteUrl, array $params): array
    {
        ...
        return Cache::remember($cacheKey, 3600, function () use ($siteUrl, $params) {
    ...
    private function finalizeSearchConsoleData(array &$data): void
    {
        ...
        // Sort by clicks
        uasort($data['queries'], fn($a, $b) => $b['clicks'] <=> $a['clicks']);
        uasort($data['pages'], fn($a, $b) => $b['clicks'] <=> $a['clicks']);
    ...
    public function getAnalyticsData(string $propertyId, array $params): array
    {
        ...
        return Cache::remember($cacheKey, 3600, function () use ($propertyId, $params) {
    ...
    private function fetchAnalyticsData(string $propertyId, array $params): array
    {
        try {
            ...
            $metrics = array_map(function($name) {
                return new \Google\Service\AnalyticsData\Metric(['name' => $name]);
            }, $metricNames);
            ...
            $dimensions = array_map(function($name) {
                return new \Google\Service\AnalyticsData\Dimension(['name' => $name]);
            }, $dimensionNames);
    ...
    public function validateCredentials(): array
    {
        ...
        try {
            ...
            $validation['errors'][] = 'Credential validation failed: ' . $e->getMessage();
        }
    ...
    public function getRateLimitInfo(): array
    {
        return [
            'search_console' => [
                ...
                'current_usage' => $this->getCurrentUsage('search_console'),
            ],
            'analytics' => [
                'requests_per_day' => 50000,
                'requests_per_hour' => 2000,
                'current_usage' => $this->getCurrentUsage('analytics'),
            ],
        ];
    }
     ...
     */
    private function getCurrentUsage(string $service): array
    {
        return [
            'requests_today' => 0,
