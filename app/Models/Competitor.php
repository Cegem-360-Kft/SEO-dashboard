...
final class Competitor extends Model
{
    ...
    public function getCompetitiveStrength(): string
    {
        ...
        if ($this->domain_authority > 70) {
            $score += 3;
        } elseif ($this->domain_authority > 50) {
            $score += 2;
        } elseif ($this->domain_authority > 30) {
            $score += 1;
        }

        if ($this->estimated_traffic > 100000) {
            $score += 3;
        } elseif ($this->estimated_traffic > 10000) {
            $score += 2;
        } elseif ($this->estimated_traffic > 1000) {
            $score += 1;
        }

        if ($this->visibility_score > 80) {
            $score += 2;
        } elseif ($this->visibility_score > 50) {
            $score += 1;
        }
    ...
    #[Scope]
    protected function byDomain($query, string $domain)
    {
